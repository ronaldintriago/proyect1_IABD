...
# FECHA "TRAMPA" PARA VER BACKLOG: "2025-12-17"
# FECHA CORRECTA PARA QUE TODO ENTRE: "2025-12-15"
...
class LogisticsController:
    ...
    def ejecutar_calculo_diario(flota_usuario):
        ...
        if not df_accepted.empty:
            ...
            # Usamos tqdm pero añadimos flush a los prints internos por si acaso
            ...
            for cluster_id in barra_progreso:
                ...
                try:
                    solver = RouteSolver(
                        ...
                        max_hours=1000, # Tiempo infinito (Tacógrafo)
                        start_date_str=FECHA_SIMULACION
                    )
                    
                    # --- AQUÍ RECIBIMOS LA LISTA DE DESCARTADOS ---
                    ruta, olvidados = solver.solve()
                    
                    # 1. Sumamos al backlog los que no se visitaron
                    if olvidados:
                        backlog_tiempo_ids.extend(olvidados)

                    # 2. Guardamos ruta válida (si tiene clientes, >2 puntos)
